...
title: |
  | ICMBio/CENAP
  | Programa Monitora - Análise de tendências populacionais de mamíferos e aves terrestres de médio e grande porte: Resex Cazumbá-Iracema, 2014-2020
...
library(here)
source("/home/elildojr/Documents/r/Programa-Monitora-Florestal-Global/experimental/cazumba_2014-2020.R")
```
...
Foram analisados dados do protocolo básico coletados na Resex Cazumbá-Iracema entre os anos de 2014 e 2020^[Os dados foram coletados, compilados e validados por monitores locais, gestores, pesquisadores do ICMBio e instituições parceiras e voluntários.]. O protocolo utiliza o método de transecções lineares [@Peres1999; @Peres2011]. Resumidamente, dois observadores percorrem as transecções a uma velocidade constante e registram, para cada avistamento de animais dos grupos-alvo, a espécie, o número de indivíduos e a distância perperndicular entre o primeiro indivíduo avistado e o centro da trilha. Cada UC deve ter pelo menos três transectos de 5 km e o esforço anual mínimo, combinando todos os transectos, deve ser de 150 km percorridos [@Nobre2015].
...
A tabela 1 apresenta o esforço amostral e o número total de registros obtidos na Resex Cazumbá-Iracema desde o início do monitoramento. Foram registrados 43 gêneros de mamiferos e aves de médio e grande porte (Tabela 2), sendo que 12 deles atenderam ao critério mínimo para inclusão nas análises: *Callicebus, Crypturellus, Dasyprocta, Guerlinguetus, Myoprocta, Pecari, Penelope, Saguinus, Saimiri, Sapajus, Tinamus, Urosciurus*.

A tabela 3 apresenta as taxas médias de crescimento estimadas para os gêneros selecionados. Todos apresentaram tendência de estabilidade (o intervalo de confiança de *r* inclui o zero) e nenhuma apresentou probabilidade alta de declínio.
...
knitr::kable(
  tabela1,
  caption = 'Esforço anual e número de registros de mamíferos obtidos pelo protocolo básico do Programa Monitora na Resex Cazumbá-Iracema, 2014-2019', booktabs = T, align = c("l", "c", "c", "c")
  ) %>%
...
```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimadas para *Callicebus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
...
```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimadas para *Crypturellus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[2]])
```


```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Dasyprocta* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[3]])
```


```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Guerlinguetus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[4]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Myoprocta* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[5]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Pecari* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[6]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Penelope* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[7]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Saguinus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[8]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Saimiri* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[9]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Sapajus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[10]])
```


```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Tinamus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[11]])
```


```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual estimada para *Urosciurus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[12]])
```
