...
# para os testes usar dados de tumucumaque-Iracema
tumucumaque <- subset(dadosICMBio, nome.UC == "Parna Montanhas do Tumucumaque")

# remover primeiro ano de dados porque esforco foi baixo
unique(tumucumaque$Ano)
tumucumaque <- tumucumaque %>% 
  filter(Ano != 2014)

# ... e somente mamiferos 
#tumucumaque <- subset(tumucumaque, Classe == "Mammalia")
dim(tumucumaque)

# tabela 1 com esforco por ano
tabela1 <- tibble(
  'Ano' = sort(unique(tumucumaque$Ano)),
  'Transectos' = 0,
  'Esforço (km)' = 0,
  'Registros' = 0
)
for(i in 1:nrow(tabela1)) {
  df1 <- subset(tumucumaque, Ano == unique(tumucumaque$Ano)[i])
...
# tabela para o anexo
tabela2 <- tumucumaque %>% group_by(Genero) %>% summarize(n = n(), ) %>% arrange(Genero)
tabela2
tabela2$'grupos/10km' <- round((tabela2$n/sum(tumucumaque$esforço, na.rm=T))*10000, 2)
print(tabela2, n=Inf)
...
encounter.rate(tumucumaque, "Genero")
#encounter_rate_tumucumaque
encounter_rate
...
encounter_rate$mean <- rowMeans(encounter_rate[,c(3:7)], na.rm = TRUE)
...
for(i in 1:length(species)) {
  ...
  # plot trends
  jpeg(here("report", paste(gsub(" ", "_", species)[i], "tumucumaque.jpg", sep="_")), width=1000, height=600, res=120) # Open jpeg file
  pop.trends()
  dev.off()
...
,,tabela3
#write.csv(tabela3, here("experimental", "tabela3_tumucumaque.csv"))
#class(tabela3)
...
#library("xtable")
#print(xtable(tabela3), type="html", file=here("experimental", "tabela3_tumucumaque.html"))
