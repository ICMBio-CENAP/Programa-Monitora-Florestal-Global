...
# para os testes usar dados de jamari-Iracema
jamari <- subset(dadosICMBio, nome.UC == "Flona do Jamari")

# ... e somente mamiferos 
jamari <- subset(jamari, Classe == "Mammalia")
#jamari[jamari$Especie == "Dasyprocta cristata", "Especie"] <- "Dasyprocta fuliginosa"
#jamari[jamari$Especie == "Dasyprocta leporina", "Especie"] <- "Dasyprocta fuliginosa"
#jamari[jamari$Especie == "Dasyprocta sp.", "Especie"] <- "Dasyprocta fuliginosa"
#head(jamari)
dim(jamari)

# tabela 1 com esforco por ano
tabela1 <- tibble(
  'Ano' = sort(unique(jamari$Ano)),
  'Transectos' = 0,
  'Esforço (km)' = 0,
  'Registros' = 0
)
for(i in 1:nrow(tabela1)) {
  df1 <- subset(jamari, Ano == unique(jamari$Ano)[i])
...
# tabela para o anexo
tabela2 <- jamari %>% group_by(Especie) %>% summarize(n = n(), ) %>% arrange(Especie)
tabela2
tabela2$'grupos/10km' <- round((tabela2$n/sum(jamari$esforço, na.rm=T))*10000, 2)
print(tabela2, n=Inf)
...
encounter.rate(jamari, "Especie")
#encounter_rate_jamari
encounter_rate
...
for(i in 1:length(species)) {
  ...
  # plot trends
  jpeg(here("report", paste(gsub(" ", "_", species)[i], "jamari.jpg", sep="_")), width=1000, height=600, res=120) # Open jpeg file
  pop.trends()
  dev.off()
...
tabela3
#write.csv(tabela3, here("experimental", "tabela3_jamari.csv"))
class(tabela3)
...
#library("xtable")
#print(xtable(tabela3), type="html", file=here("experimental", "tabela3_jamari.html"))
