...
# para os testes usar dados de juruena
juruena <- subset(dadosICMBio, nome.UC == "Parna do Juruena")

# ... e somente mamiferos 
juruena <- subset(juruena, Classe == "Mammalia")
#juruena[juruena$Especie == "Dasyprocta cristata", "Especie"] <- "Dasyprocta fuliginosa"
#juruena[juruena$Especie == "Dasyprocta leporina", "Especie"] <- "Dasyprocta fuliginosa"
#juruena[juruena$Especie == "Dasyprocta sp.", "Especie"] <- "Dasyprocta fuliginosa"
#head(juruena)
dim(juruena)

# tabela 1 com esforco por ano
tabela1 <- tibble(
  'Ano' = sort(unique(juruena$Ano)),
  'Transectos' = 0,
  'Esforço (km)' = 0,
  'Registros' = 0
)
for(i in 1:nrow(tabela1)) {
  df1 <- subset(juruena, Ano == unique(juruena$Ano)[i])
...
# tabela para o anexo
tabela2 <- juruena %>% group_by(Especie) %>% summarize(n = n(), ) %>% arrange(Especie)
tabela2
tabela2$'grupos/10km' <- round((tabela2$n/sum(juruena$esforço, na.rm=T))*10000, 2)
print(tabela2, n=Inf)
...
encounter.rate(juruena, "Especie")
#encounter_rate_juruena
encounter_rate
...
encounter_rate <- encounter_rate[,-7]
encounter_rate
#head(encounter_rate)
dim(encounter_rate)
...
for(i in 1:length(species)) {
  ...
  # plot trends
  jpeg(here("report", paste(gsub(" ", "_", species)[i], "juruena.jpg", sep="_")), width=1000, height=600, res=120) # Open jpeg file
  pop.trends()
  dev.off()
...
tabela3
#write.csv(tabela3, here("experimental", "tabela3_juruena.csv"))
class(tabela3)
...
#library("xtable")
#print(xtable(tabela3), type="html", file=here("experimental", "tabela3_juruena.html"))
