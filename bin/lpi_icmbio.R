...
lpi_icmbio <- function(x,y,z) # x = dados, y = UC, z = Classe
{
  ...
  for.effort.1 <- mydata # sera usado mais a frente
  ...
  vetor.Ano <- sort(unique(mydata$Ano)) # sera necessario abaixo
  ...
  for(i in 1:nrow(mydata2))
    for(j in 1:length(vetor.Ano)){
      ...
      for.effort.2 <- filter(for.effort.1, CDUC %in% b$CDUC)
      #mydata2[i,j+2] <- nrow(b)/(sum(mydata$esforço, na.rm=TRUE))
      mydata2[i,j+2] <- nrow(b)/(sum(subset(for.effort.2, Ano == vetor.Ano[j])$esforço, na.rm=TRUE))*10000 # o esforco usa a planilha mydata porque so uma linha por censo tem esforco
    }
  
  mydata2[is.nan(mydata2)] <- 0
