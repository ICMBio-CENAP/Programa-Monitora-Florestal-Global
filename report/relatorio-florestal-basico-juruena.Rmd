...
title: |
  | ICMBio/CENAP
  | Programa Monitora - Análise de tendências populacionais de mamíferos terrestres de médio e grande porte: Resex Cazumbá-Iracema, 2014-2019
...
A tabela 1 apresenta o esforço amostral e o número total de registros obtidos na Resex Cazumbá-Iracema desde o início do monitoramento. Foram registradas 36 especies de mamiferos de médio e grande porte^[a lista completa está disponível no anexo 1], sendo que 11 delas atenderam ao critério mínimo para inclusão nas análises: *Alouatta puruensis*, *Aotus nigriceps*, *Callicebus cupreus*, *Cebus unicolor*, *Dasyprocta fuliginosa*, *Guerlinguetus ignitus*, *Myoprocta pratti*, *Pecari tajacu*, *Saimiri boliviensis*, *Sapajus macrocephalus*, *Urosciurus spadiceus*.

A maioria das espécies apresentou taxa de crescimento negativa (Tabela 1, Figuras 1 a 11), mas os intervalos de confiança foram muito amplos e em todos os casos incluem o zero, ou seja, as tendências de declínio não foram estatisticamente significativas. *Isso não significa que não tenham ocorrido declínios*, mas sim que, caso tenham ocorrido declínios, eles não foram suficientemente fortes para serem detectados pelo modelo. Essa incerteza decorre da variabilidade natural do sistema e das limitações do índice populacional e da análise, que pode não ter poder estatístico suficiente para detectar tendências populacionais sutis, ao menos em prazos muito curtos [@Fairweather1991]. Apesar da incerteza das estimativas, pelo menos três espécies apresentaram alta probabilidade (> 75%) de estarem em declínio: *Alouatta puruensis*, *Aotus nigriceps* e *Callicebus cupreus*. Estas espécies merecem atenção especial.
...
Em resumo, foram avaliadas as tendências populacionais de 11 espécies de mamíferos de médio e grande porte na Resex Cazumbá-Iracema entre os anos de 2014 e 2019. A análise não detectou declínios estatisticamente significativos, embora algumas espécies tenham apresentado alta probabilidade de estarem em declínio. Para uma interpretação adequada desses padrões, recomenda-se que esses resultados sejam debatidos entre comunidades locais, gestores e pesquisadores.
...
```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Alouatta puruensis* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
...
```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Aotus nigriceps* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[2]])
```


```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Callicebus cupreus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[3]])
```


```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Cebus unicolor* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[4]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Dasyprocta fuliginosa* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[5]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Guerlinguetus ignitus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[6]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Myoprocta pratti* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[7]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Pecari tajacu* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[8]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Saimiri boliviensis* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[9]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Sapajus macrocephalus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
  pop.trends.for.rmd(N.est_all_species[[10]])
```



```{r, echo=FALSE, message=FALSE, fig.cap="Taxa de encontro anual para *Urosciurus spadiceus* na Resex Cazumbá-Iracema. As barras verticais indicam a incerteza associada à estimativa (IC de 95%)"}
