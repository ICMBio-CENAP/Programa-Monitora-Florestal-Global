...
... populações = 0 travam o modelo, por isso zeros foram resetados para 0.001 ... afinal zero no indice populacional não significa zero na população e mesmo que significasse poderia haver recolonização ... resear para 0.001 também tem a vantagem de permitir incluir especies em declino e que pararam de ser detectadas no meio do monitoramento, afinal o criterio de seleção de espécies pode estar justamente removendo as spp mais interessantes... nao esquecer de resetar N minimo no modelo do jags para zero 


*Processo de observação*

As observações estão sujeitas a erro estocástico ($\epsilon$) com distribuição normal centrada no zero:
...
O modelo foi implementado numa abordagem Bayesiana, seguindo as especificações de [@kery2011bayesian] (p. 126-130) no JAGS [@Plummer2015] usando o pacote R2jags [@su2012r2jags]. Foram executadas três Cadeias de Markov e Monte Carlo (MCMC) com 25000 iterações, *burn-in* de 1000 e *thinning rate* de 3. A convergência dos parâmetros foi avaliada pelo diagnóstico de Gelman-Rubin [@Gelman2011]. Todos os dados e códigos utilizados estão disponiveis no repositório https://github.com/ICMBio-CENAP/Programa-Monitora-Florestal-Global.
